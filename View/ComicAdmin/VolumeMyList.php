...
$(function(){
	...
	$(".assess-star").click(function(){
		var star_num = $(this).attr("star-num");
	...
	<div class="volume-list-box">
		...
		<?php foreach ($v->volume_list as $key => $volume): ?>
			<li class="volume-data" book_id = <?= $volume['book_id']?>>
				<div class="book-name"><?= $volume['book_name']?></div>
				...
				<div class="assessment">star
					<?php for($i=0; $i<$volume['assessment']; $i++):?><div class="star-img"></div> <?php endfor;?>
				</div>
				<div class="comment">
					<?=$volume['user_comment']?>
				</div>
				...
				<div class="dialog" id="dialog<?= $volume['book_id']?>" title="edit volume data">
					...
					<div class="assessment">star: 
						<?php for($i=0;$i<5; $i++):?>
							<div class="ui-icon ui-icon-star assess-star" star-num="<?=$i?>"></div>
						<?php endfor; ?>
					</div>
